function initApp() {
    ...
    const searchSuggestions = getElement('searchSuggestions');
    const tagScrollLeft = getElement('tagScrollLeft');
    const tagScrollRight = getElement('tagScrollRight');
    const tagsScroll = getElement('tagsScroll');
    const listingsGrid = getElement('listingsGrid');
    const loadMoreBtn = getElement('loadMoreBtn');
    const backToTop = getElement('backToTop');
    
    // Get view and filter buttons (these might not exist on all pages)
    const viewButtons = document.querySelectorAll('.view-btn');
    const filterButtons = document.querySelectorAll('.filter-btn');

    // Only proceed if we have essential elements (navbar elements might not be loaded yet)
    if (!navbar) {
    ...
    // Search Suggestions (only if elements exist)
    ...
    // Tags Scroll (only if elements exist)
    if (tagScrollLeft && tagScrollRight && tagsScroll) {
    ...
    // View Toggle (only if buttons exist)
    if (viewButtons.length > 0 && listingsGrid) {
    ...
    // Filter Listings (only if buttons exist)
    ...
    // Load More Listings (only if button exists)
    ...
    // Back to Top Button (only if button exists)
    if (backToTop) {
